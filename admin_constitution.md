...
- **Decomposer** — breaks a claim into the subclaims and arguments that bear on it, purely structurally. It maps dependencies; it neither judges their truth nor decides whether they already exist (that is the Matcher's call).

- **Claim Steward** — the owner of a single claim's page, end to end: its canonical form, its decomposition and arguments, and—centrally—its **assessment**. Assessing a claim is open-ended judgment, so it belongs to the agent that owns the claim over time rather than to a fire-once scorer. The Steward may consult anything it needs—the claim's subclaims and their assessments, related claims elsewhere in the graph, and outside evidence via search—and reaches a holistic verdict whose depth scales with the claim's importance. Because assessment is provisional, the Steward re-judges as evidence accrues and as the claims it depends on change.
